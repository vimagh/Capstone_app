...
model2 = pickle.load(open('pickledUpper_Gbr.pkl', 'rb'))
...
def main():
    ...
    Southern_Asia = st.text_input('Southern Asia')
    Sub_Saharan_Africa = st.text_input('Sub Saharan Africa')
...
#prediction code
    ...
    if st.button('Upper Secondary'):
            makeprediction = model2.predict([[Childhood_Education_GER, Gross_enrolment_ratio_early_childhood_educational_development_programmes, 
                                              Literacy_rate_for_25_64_years_old, Expenditure_on_education_as_a_percentage_of_total_government_expenditure,
                                              Government_expenditure_on_education_as_a_percentage_of_GDP, Central_Asia,
                                              Central_and_Southern_Asia, Eastern_and_South_Eastern_Asia, Europe_and_Northern_America,
                                              Latin_America_and_the_Caribbean, Northern_Africa_and_Western_Asia, Oceania, Southern_Asia,
                                              Sub_Saharan_Africa, Gender_numerical]])
            output = round(makeprediction[0],2)
            st.success('Upper Secondary Completion Rate {}'.format(output))
            
    if st.button('Lower Secondary'):
                makeprediction = model2.predict([[Childhood_Education_GER, Gross_enrolment_ratio_early_childhood_educational_development_programmes,
                                                  Literacy_rate_for_25_64_years_old, Expenditure_on_education_as_a_percentage_of_total_government_expenditure,
                                                  Government_expenditure_on_education_as_a_percentage_of_GDP, Central_Asia,
                                                  Central_and_Southern_Asia, Eastern_and_South_Eastern_Asia, Europe_and_Northern_America,
                                                  ...
                                                  Sub_Saharan_Africa, Gender_numerical]])
                output = round(makeprediction[0],2)
                st.success('Lower Secondary Completion Rate {}'.format(output))
